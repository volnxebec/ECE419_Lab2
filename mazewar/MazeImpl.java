...
public class MazeImpl extends Maze implements Serializable, ClientListener, Runnable {
        ...
        public synchronized boolean clientProjectileMove(MPacket pkt) {
          ...
          if (!projectileMap.isEmpty()) {
            ...
            synchronized(projectileMap) {
              while(it.hasNext()) {
                ...
                if (pkt.name.equals(prj.getOwner().getName()) && 
                    pkt.projectileMoveID == prj.ID) {
                        ...
                        if(!projectileMap.isEmpty()) {
                                ...
                                synchronized(projectileMap) {
                                        while(it.hasNext()) {   
                                                ...
                                                assert(o instanceof Projectile);
                                                Projectile prj = (Projectile)o;
                                                //Skip projectiles that can't move this turn
                                                if (!prj.getMove()) {
                                                  System.out.println("Client "+myName+" can't move pkt with owner " + 
                                                                      prj.getOwner().getName()+" with ID="+prj.ID);
                                                  continue;
                                                }
                                                deadPrj.addAll(moveProjectile(prj));
